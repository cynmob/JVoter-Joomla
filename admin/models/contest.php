 ...
 * @since  1.6
 */
class JVoterModelContest extends JModelAdmin
{
    /**
     ...
     * @since  3.2
     */
    public $typeAlias = 'com_jvoter.contest'; 
    ...
    protected function canDelete($record)
    {
        if (!empty($record->id))
        {
            ...
            return JFactory::getUser()->authorise('core.delete', 'com_jvoter.contest.' . (int) $record->id);
        }
    ...
    protected function canEditState($record)
    {
        ...
        if (!empty($record->id))
        {
            return $user->authorise('core.edit.state', 'com_jvoter.contest.' . (int) $record->id);
        }
        
        // New article, so check against the category.
        if (!empty($record->catid))
        {
            return $user->authorise('core.edit.state', 'com_jvoter.category.' . (int) $record->catid);
        }
    ...
    protected function prepareTable($table)
    {
        ...
        // Reorder the contests within the category so the new article is first
        if (empty($table->id))
        {
     ...
     */
    public function getTable($type = 'Contest', $prefix = 'JVoterTable', $config = array())
    {
    ...
    {
        // Get the form.
        $form = $this->loadForm('com_jvoter.contest', 'contest', array('control' => 'jform', 'load_data' => $loadData));
        ...
        if ($this->getState('contest.id'))
        {
            $id = $this->getState('contest.id');
        ...
        if ($id != 0 && (!$user->authorise('core.edit.state', 'com_jvoter.contest.' . (int) $id))
    ...
    public function save($data)
    {
        ...
        if ($catid > 0)
        {
            $catid = CategoriesHelper::validateCategoryId($data['catid'], 'com_jvoter');
        }
        ...
        if (in_array($input->get('task'), array('apply', 'save', 'save2new')) && (!isset($data['id']) || (int) $data['id'] == 0))
        {
            if ($data['alias'] == null)
            {
                ...
                {
                    $msg = JText::_('COM_JVOTER_SAVE_WARNING');
                }
     ...
     * @since   3.0
     */
    protected function preprocessForm(JForm $form, $data, $group = 'contest')
    {
    ...
    /**
     * Custom clean the cache of com_jvoter and contest modules
     *
